...
function val(f,type){if(type==='quota')return TradeInfo.amount(f);if(type==='return')return f.returns?.[years];if(type==='drawdown')return f.drawdowns?.[years]==null?undefined:Math.abs(f.drawdowns[years]);if(type==='fee')return f.fees?.management;if(type==='purchase_fee'){const match=String(f.purchase_fee??'').trim().match(/^(\d+(?:\.\d+)?)%$/);return match?Number(match[1]):undefined}return f[type]}
function activeFilters(f){const q=$('search').value.toLowerCase().trim();const c=$('channel').value,fc=f.channel||'unknown';if(c!=='all'&&!(c===fc||(['exchange','otc'].includes(c)&&fc==='both')))return false;if(category!=='all'&&f.category!==category)return false;if($('currency').value!=='all'&&f.currency!==$('currency').value)return false;if(q&&!`${f.name} ${f.code} ${f.target||''}`.toLowerCase().includes(q))return false;if($('available').checked&&(!['otc','both'].includes(fc)||!['开放申购','限大额'].includes(f.purchase_status)))return false;if($('starred').checked&&!favorites.has(f.code))return false;for(const [id,value,direction] of [['minQuota',TradeInfo.amount(f),1],['minReturn',f.returns?.[years],1],['maxFee',f.fees?.management,-1],['maxPremium',f.premium,-1]]){const s=$(id).value;if(s!==''&&(value==null||(direction===1?value<Number(s):value>Number(s))))return false}return true}
...
function quotaCell(f){
 const t=TradeInfo.info(f),info=f.field_states?.quota||{};
 const retry=t.retry?missingCell(f,'quota','待核验'): `<strong class="trade-${t.tone}">${esc(t.otc)}</strong>`;
 return `<div class="trade-info"><div class="trade-line" title="场外限额：来源平台口径；获取时间 ${esc(stamp(info.checked_at))}"><span>场外申购</span>${retry}</div><div class="trade-line" title="场内申购需另查当日清单或公告，不从场外状态推断"><span>场内申购</span><strong class="trade-muted">${t.primary}</strong></div><div class="trade-line" title="上市标记不代表当前未停牌，请以交易所及券商状态为准"><span>场内交易</span><strong>${t.trading}</strong></div></div>`;
}
function premiumCell(f){
 const value=metricCell(f,'premium',f.premium==null?null:pct(f.premium),f.quote_date,'暂无有效 IOPV');
 if(!f.listed)return value;
 return `<div class="premium-cell">${f.premium!=null?`<button class="premium-history-trigger" data-premium-history="${f.code}" aria-label="查看 ${esc(f.name)} 的场内溢价历史">${pct(f.premium)}</button>${f.quote_date?`<small>${esc(f.quote_date)}</small>`:''}`:value}${f.premium==null?`<button class="premium-history-link" data-premium-history="${f.code}">查看历史</button>`:''}</div>`;
}
function navCell(f){
 const value=metricCell(f,'nav',f.nav==null?null:f.nav.toFixed(4),f.nav_date,'净值未披露');
 const current=f.nav!=null&&!['error','loading'].includes(f.field_states?.nav?.status);
 return `<div class="nav-cell">${current?`<button class="nav-history-trigger" data-nav-history="${f.code}" aria-label="查看 ${esc(f.name)} 的历史净值">${f.nav.toFixed(4)}</button>${f.nav_date?`<small>${esc(f.nav_date)}</small>`:''}`:value}${!current?`<button class="nav-history-link" data-nav-history="${f.code}">查看历史</button>`:''}</div>`;
}
function fieldCards(f){return [
 ...
 ].map(([key,value,sub])=>`<div class="metric"><span>${fieldLabels[key]}${['returns','drawdowns'].includes(key)?' · '+years+' 年':''}</span><div class="metric-value">${key==='nav'?navCell(f):metricCell(f,key,value,sub,['returns','drawdowns'].includes(key)?'年限 / 复权待核验':'来源未提供')}</div></div>`).join('')}
...
function render(){
 ...
 filtered=dataset.funds.filter(activeFilters);filtered.sort((a,b)=>{if(sortKey==='quota')return TradeInfo.compare(a,b,sortDirection);const av=val(a,sortKey),bv=val(b,sortKey);if(av==null)return bv==null?a.code.localeCompare(b.code):1;if(bv==null)return -1;return (sortDirection==='desc'?bv-av:av-bv)||a.code.localeCompare(b.code)});
 ...
 const sortLabels={nav:'基金净值',return:years+' 年年化',drawdown:years+' 年最大回撤',quota:'场外日限额',purchase_fee:'申购费',fee:'管理费',premium:'场内溢价'};
 ...
 $('sortStatus').textContent=`${sortLabels[sortKey]} · ${sortDirection==='asc'?'升序 ↑':'降序 ↓'}${sortKey==='drawdown'?'（按回撤幅度）':sortKey==='quota'?'（人民币、美元分组）':''} · 点击表头切换方向，非数值项始终置后`;
 $('rows').innerHTML=filtered.slice((page-1)*15,page*15).map(f=>{const r=f.returns?.[years],dd=f.drawdowns?.[years];return `<tr><td><button class="fund-name" data-detail="${f.code}">${esc(f.name)}</button><div class="fund-meta"><span class="code">${f.code}</span><span class="tag ${f.category}">${labels[f.category]||'待核验'}</span><span class="tag channel-tag" title="按份额类型初分；是否开放以最新公告及销售渠道为准">${channelLabels[f.channel]||channelLabels.unknown}</span></div>${['error','missing','loading'].includes(f.field_states?.structure?.status)?missingCell(f,'structure','结构待核验'):''}</td><td>${navCell(f)}</td><td class="${r==null?'dim':r>=0?'positive':'negative'}">${metricCell(f,'returns',r==null?null:`${r>0?'+':''}${pct(r)}`,f.returns_date,'年限 / 复权待核验')}</td><td class="${dd==null?'dim':'negative'}">${metricCell(f,'drawdowns',dd==null?null:pct(dd),f.drawdowns_date,'年限 / 复权待核验')}</td><td>${quotaCell(f)}</td><td>${metricCell(f,'purchase_fee',f.purchase_fee?esc(f.purchase_fee):null)}</td><td>${metricCell(f,'management',f.fees?.management==null?null:pct(f.fees.management))}</td><td class="${f.premium!=null&&f.premium>3?'negative':'dim'}">${premiumCell(f)}</td><td><button class="star ${favorites.has(f.code)?'saved':''}" data-star="${f.code}" aria-label="${favorites.has(f.code)?'取消关注':'关注'} ${esc(f.name)}" aria-pressed="${favorites.has(f.code)}">${favorites.has(f.code)?'★':'☆'}</button></td></tr>`}).join('');
 document.querySelectorAll('#rows tr').forEach(row=>{
 const names=['基金', '基金净值', years+' 年年化',years+' 年最大回撤','申购与交易','申购费','管理费 / 年','场内溢价','关注'];
...
function historyChart(f){
 return `<section class="detail-section nav-history-section" data-nav-detail="${f.code}"><div class="nav-history-head"><h3>历史单位净值</h3><span>估值日 · 非实时行情</span></div><div class="nav-detail-body">正在读取历史净值…</div></section>`;
}
