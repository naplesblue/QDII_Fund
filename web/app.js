...
function quota(f){if(f.purchase_status==='暂停申购')return '暂停申购';if(f.purchase_status==='场内交易')return '场内交易';if(!['限大额','开放申购'].includes(f.purchase_status))return f.purchase_status||'待核验';return f.quota?`${f.currency==='USD'?'$':'¥'}${f.quota.toLocaleString('zh-CN')}`:'额度待核验'}
...
function activeFilters(f){const q=$('search').value.toLowerCase().trim();if(category!=='all'&&f.category!==category)return false;if($('currency').value!=='all'&&f.currency!==$('currency').value)return false;if(q&&!`${f.name} ${f.code} ${f.target||''}`.toLowerCase().includes(q))return false;if($('available').checked&&!['开放申购','限大额'].includes(f.purchase_status))return false;if($('starred').checked&&!favorites.has(f.code))return false;for(const [id,value,direction] of [['minQuota',f.quota,1],['minReturn',f.returns?.[years],1],['maxFee',f.fees?.management,-1],['maxPremium',f.premium,-1]]){const s=$(id).value;if(s!==''&&(value==null||(direction===1?value<Number(s):value>Number(s))))return false}return true}
const fieldLabels={nav:'基金净值',returns:'年化收益',drawdowns:'最大回撤',quota:'每日额度',purchase_fee:'申购费',management:'管理费',custody:'托管费',service:'销售服务费',redemption:'赎回费',structure:'投资结构',premium:'场内溢价'};
...
function render(){
 ...
 const sortLabels={nav:'基金净值',return:years+' 年年化',drawdown:years+' 年最大回撤',quota:'每日额度',purchase_fee:'申购费',fee:'管理费',premium:'场内溢价'};
...
function reset(){category='all';page=1;['search','minQuota','minReturn','maxFee','maxPremium'].forEach(id=>$(id).value='');$('currency').value='RMB';$('available').checked=false;$('starred').checked=false;sortKey='return';sortDirection='desc';render()}$('reset').onclick=reset;$('emptyReset').onclick=reset;
...
loadData().then(checkStatus).catch(async()=>{try{const r=await fetch('data.json');if(!r.ok)throw Error();dataset=await r.json();render();message('只读快照模式。手动刷新需通过 python3 server.py 启动本地服务。',true)}catch{message('未能加载数据。请在项目目录运行 python3 server.py，然后访问 http://127.0.0.1:8765。',true)}});
