...
(()=>{
 ...
 const cache=new Map();let anchor=null,code='',years=3,pinned=false,openTimer,closeTimer;
 ...
 async function read(fund,period){const key=fund+':'+period,old=cache.get(key);if(old&&Date.now()-old.loaded<60000)return old.data;
  const response=await fetch(`/api/nav-history?code=${fund}&years=${period}`,{cache:'no-store'});if(!response.ok)throw Error('历史读取失败');
  const data=await response.json();cache.set(key,{data,loaded:Date.now()});return data;
 }
 function controls(period){return `<div class="nav-periods" role="group" aria-label="净值区间">${[1,3,5].map(n=>`<button type="button" data-nav-years="${n}" aria-pressed="${n===period}">${n} 年</button>`).join('')}</div>`;}
 ...
 function draw(target,raw,large){
  ...
  target.innerHTML=`<div class="nav-inspect" aria-live="polite">${inspect(points.at(-1))}</div><svg class="nav-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="单位净值逐日折线图；长于十四天的缺口不连线">${grid}<g class="nav-line">${paths}</g><g class="nav-isolated">${isolated}</g><g class="nav-events">${events}</g><line class="nav-cross" y1="${top}" y2="${h-bottom}" hidden/><circle class="nav-focus" r="5" hidden/><text x="${l}" y="${h-6}">${points[0][0]}</text><text x="${w-r}" y="${h-6}" text-anchor="end">${points.at(-1)[0]}</text></svg><p class="nav-sample">${points.length} 个估值日${points.length===1?' · 仅一个样本，尚不能形成曲线':''} · 橙点表示来源标记的分红 / 拆分</p>`;
  const svg=target.querySelector('svg'),cross=svg.querySelector('.nav-cross'),focus=svg.querySelector('.nav-focus'),label=target.querySelector('.nav-inspect');
  svg.addEventListener('pointermove',event=>{const box=svg.getBoundingClientRect(),coord=(event.clientX-box.left)/box.width*w,at=dates[0]+Math.max(0,Math.min(1,(coord-l)/(w-l-r)))*(dates.at(-1)-dates[0]);
   ...
   cross.setAttribute('x1',x(i));cross.setAttribute('x2',x(i));cross.hidden=false;focus.setAttribute('cx',x(i));focus.setAttribute('cy',y(values[i]));focus.hidden=false;label.innerHTML=inspect(points[i]);
  });
 }
 ...
 function renderPanel(){panel.innerHTML=`<div class="nav-popover-head"><strong>${code} · 历史单位净值</strong><button type="button" data-nav-close aria-label="关闭净值历史">×</button></div>${controls(years)}<div class="nav-chart-body"></div><p class="nav-help">悬停图线查看估值日和来源日涨跌。点击净值可固定小窗。</p>`;panel.hidden=false;position();fill(panel.querySelector('.nav-chart-body'),code,years,false).finally(position);}
 ...
 function mountDetail(fund,period){const section=document.querySelector(`[data-nav-detail="${fund}"]`);if(!section)return;
  section.innerHTML=`<div class="nav-history-head"><h3>历史单位净值</h3><span>估值日 · 非实时行情</span></div>${controls(Number(period))}<div class="nav-detail-body"></div>`;
  fill(section.querySelector('.nav-detail-body'),fund,Number(period),true);
 }
 ...
 document.addEventListener('click',e=>{const button=e.target.closest('[data-nav-history]');if(button){open(button,true);return;}if(!panel.contains(e.target))close();});
 document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!panel.hidden)close();});
 panel.addEventListener('click',e=>{if(e.target.closest('[data-nav-close]')){close();return;}const period=e.target.closest('[data-nav-years]');if(period){years=Number(period.dataset.navYears);pinned=true;renderPanel();}if(e.target.closest('[data-nav-retry]'))renderPanel();});
 document.addEventListener('click',e=>{const period=e.target.closest('[data-nav-detail] [data-nav-years]');if(period)mountDetail(period.closest('[data-nav-detail]').dataset.navDetail,period.dataset.navYears);const retry=e.target.closest('[data-nav-detail] [data-nav-retry]');if(retry)mountDetail(retry.closest('[data-nav-detail]').dataset.navDetail,retry.closest('[data-nav-detail]').querySelector('[aria-pressed=true]').dataset.navYears);});
